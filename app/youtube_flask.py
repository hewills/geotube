...
class YouTubeSearch:
    ...
    def search(self):
        ...
        while page_count < MAX_PAGES:
            ...
            # ------------------- VIDEO DETAILS -------------------
            video_parts = ["snippet"]
            need_statistics = self.max_views is not None or self.max_comments is not None or self.max_subscribers is not None
            if need_statistics:
                video_parts.append("statistics")
            ...
            # Collect channel IDs for subscriber filter if needed
            subs_dict = {}
            if self.max_subscribers is not None and "statistics" in video_parts:
            ...
            for video in video_response.get("items", []):
                ...
                views = int(video["statistics"].get("viewCount", 0)) if "statistics" in video_parts else None
                comments = int(video["statistics"].get("commentCount", 0)) if "statistics" in video_parts else None
